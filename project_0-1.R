...
<<<<<<< HEAD
  ...
  residbla2 <- as.matrix(residbla2)
  DetrendLogMeasurements <- DetrendLogMeasurements - abs(residbla2)
  LogMeasurementsC <- subset(LogMeasurements[,b], Type == 'C')
  
  #COMPARE R2, PICK MODELS - pick best one for each compound? Or do all the same? Or write some code that evaluates R2, picks best model for each compound
  #DO PREDICTION FROM EVERYTHING, AND THEN SUBTRACT FROM OBSERVED (EVERYTHING)
  #THEN COMPARE CASES AND CONTROLS
  #Predict() looks for something in a dataframe with the same name as the var used in the model
  
  # Lorder1 <- subset(Lorder, Type == 'C')
  # Lorder2 <- subset(Lorder, Type == 'S')
  # Lorder2 <- as.matrix(Lorder2)
  # Lorder2 <- Lorder2[-1]
  # cat(Lorder1, "\n", Lorder2)
  #plot(Lorder, LogMeasurements[,b], main = titleLog, xlab = "Injection Order", ylab = "Log Relative Intensity", col=Type)
  # plottystuff <- rbind(LogMeasurementsC, DetrendLogMeasurements)
  # plot(Lorder, plottystuff, main = titleLog, xlab = "Injection Order", ylab = "Log Relative Intensity", col=Type)
  # plot(Lorder2, DetrendLogMeasurements)
# - Making a trendline
